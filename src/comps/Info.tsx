...
export default function Info({data, onClose}: Props){
    ...
    const pointData = data[Number(params.get("placeInfoId"))]
    useEffect(()=>{
        ...
        if (document.getElementById("hoursHandler") && pointData.openingHours)
            document.getElementById("hoursHandler")!.innerHTML = pointData.openingHours;
    },[])
    return(
        <div className={`animate-fade-in z-100 w-full h-full fixed top-0 left-0 bg-[#00000099] flex justify-center items-center duration-300 transition ${keepOpen && "opacity-0"}`}>
            <div className="w-[2000px] max-h-[3172px] bg-white rounded-[72px] p-[48px]">
                ...
                <div className="mt-[48px] w-[1904px] h-[1070.49px] rounded-[48px] overflow-hidden">
                    <img src={apiUrl + pointData.image} alt="image" className="w-full h-full object-cover" />
                </div>
                <div className="mt-[24px] grid grid-cols-2 gap-[16px]">
                    ...
                    <div className="w-[944px] h-[250px] text-center rounded-[48px] bg-[#04341C0D] p-[48px]">
                        ...
                        <div id="hoursHandler" className="text-[48px] text-text font-normal leading-[100%] mt-[16px]"/>
                ...
                {pointData.description && <div id="infoHandler" className="mt-[48px] text-text text-[40px]"/>}
