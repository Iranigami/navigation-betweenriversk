...
export default function Filters({onClose, onSetFiltered, selectedFilters}: Props){
            ...
            <div className="w-[2000px] max-h-[3172px] bg-white rounded-[72px] p-[48px] pt-[24px]">
                ...
                <div className="mt-[24px] w-[1904px]">
                    {filters.map((filter, index:number) => (
                        ...
                        onClick={() => {
                            ...
                            <img hidden={currentFilters.includes(filter)} src={checkIconInactive} alt="check" className="size-[64px]" />
                            <img hidden={!currentFilters.includes(filter)} src={checkIconActive} alt="check" className="size-[64px]" />
                            {filter}
                        </div>
                    ))}
                </div>
                <div className="mt-[48px] flex justify-center gap-[16px]">
                        <button 
                            ...
                            disabled={currentFilters.length===0} className="disabled:opacity-20 w-[944px] h-[144px] rounded-[48px] bg-light-green text-white flex justify-center items-center text-[48px] font-semibold leading-[100%]">
                            Применить
                        </button>
                        <button 
                            onClick={ () =>
