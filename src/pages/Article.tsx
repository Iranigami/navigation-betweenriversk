...
export default function Article({news}: Props){
    ...
    const testdata = news[Number(params.get("id"))]
    return(
                ...
                <div className="overflow-x-hidden overflow-y-auto rounded-[48px] w-[1904px] h-[3076px]">
                    <div className="w-[1864px]">
                        <div className="w-[1864px] h-[1048px] rounded-[48px] overflow-hidden">
                            <img src={apiUrl + testdata.image} alt="image" className="w-full h-full object-cover" />
                        </div>
                        ...
                        <div className="mt-[48px] text-text text-[40px] font-normal leading-[100%]">
                            {testdata.eventData.map((data, index:number)=>(
                                <div key={index} className="text-text text-[40px] font-normal leading-[100%] mb-[48px]">
                                    <div className="mb-[24px] text-[56px] font-bold">{data.title}</div>
                                    {data.description}
                                </div>
                            ))}
                        </div>
